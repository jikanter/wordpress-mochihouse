...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<footer class="entry-footer">
		<?php
			...
			$category_list = get_the_category_list( __( ', ', 'mochihouse' ) );
      ...
      if ((get_post_type() != 'project') && (get_post_type() != 'artist')) { 
			  if ( ! mochihouse_categorized_blog() ) {
				...
				if ( '' != $tag_list ) {
					$meta_text = __( 'This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'mochihouse' );
				} else {
